...
| **Filter Paket** | Terapkan filter UDP + DNS | `udp && ip.addr == 192.168.1.15 && dns.qry.name == "www.google.com"` | Tampilkan hanya paket yang relevan |
...
> ![DNS Query](assets/dns.png)

> **Gambar 2**: DNS Response (UDP)  
> ![DNS Response](assets/respon.png)

**Ringkasan Paket yang Tertangkap:**

| Frame | Tipe | Source | Destination | Length | Keterangan |
|-------|------|--------|-------------|--------|------------|
| 930  | DNS Query | `192.168.1.15:58869` | `182.8.64.13:53` | 74 byte | Client tanya DNS |
| 993  | DNS Response | `182.8.64.13:53` | `192.168.1.15:58869` | 239 byte | Server jawab query |

**Penjelasan Simpel:**
- Client pakai port acak `58869` untuk kirim pertanyaan
...
|-------------|------------------------|---------------------------|
| **Source Port** | 58869 (ephemeral client) | 53 (DNS server) |
| **Destination Port** | 53 (DNS server) | 58869 (ephemeral client) |
| **Length** | 74 byte | 239 byte |
| **Checksum** | `0xb806` | `0x000` |

#### 4.2.2 Perhitungan Payload UDP

| Paket | Length Total | Header UDP | Payload (Data Asli) |
|-------|-------------|------------|-------------------|
| Query | 74 byte | 8 byte | **32 byte** |
| Response | 239 byte | 8 byte | **197 byte** |
...
```
REQUEST:  192.168.1.15:58869 → 182.8.64.13:53
RESPONSE: 182.8.64.13:53   → 192.168.1.15:58869
```
...
|--------|-----------|---------------------|
| **Port Reversal** | Port source-destination dibalik saat response | Query: src=58869→dst=53 ; Response: src=53→dst=58869 |
| **Ephemeral Port** | Port sementara client (range dinamis) | Client pakai `58869` (masuk range 49152-65535) |
| **Well-Known Port** | Port standar layanan (0-1023) | DNS server selalu di port `53` |
| **Transaction ID** | ID unik untuk cocokkan query-response | DNS pakai ID `0x8d96` sama di query & response |
...
| Transaction ID | `0x8d96` | Sama di query & response → untuk matching |
